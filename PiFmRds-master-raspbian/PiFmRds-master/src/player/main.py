        ...
        for filename in files:
            if os.path.isdir(tmp+ "/"+filename):
                ...
                or filename.lower().endswith(".mp3"):
                    new = os.path.join(tmp, filename);  #将路径和查找的文件合并成路径
...
def main(path):
    ...
    pygame.display.set_caption("zhh&Jsa music player");
    #中文显示字体
    try:
    ...
    buttons["next"] = Button("next",450,525,"./image/next.png");
    buttons["fm"] = Button("fm",520,510,"./image/fm_start.png");
    ...
    for name in names:
        ...
        try:
            tmp = name[:-4].strip().decode("utf-8");   #将歌曲名***.mp3去掉后缀名并修改编码
        except :
            tmp = name[:-4].strip();
            ...
            if button_pressed is not None:
                if button_pressed == "play":
                    ...
                    if playing:
                        ...
                        first_play = True;
                        playing_index = (playing_index + 1) % max_track;
                elif button_pressed == "fm":
                     if fmworking == False :
                         buttons["fm"].setImage('./image/fm_stop.png');
                         fmworking = True;
#                         dlgtext = wx.TextEntryDialog( /
#                            self,'What is your favorite programming language?','perl','python');
#                         dlgtext.SetValue('python is good');
#                         if dlgtext.ShowModal() == wx.ID_OK:
#                            dlgmsg = wx.MessageDialog( /
#                                self,dlgtest.GetValue(),'A Message Box',wx.OK | wx.ICON_INFORMATION);
#                            dlgmsg.Center();
#                            dlgmsg.ShowModal();
#                            dlgmsg.Destroy();
#                         dlgtext.Destroy();   
                         ...
                         if playing:
                                pygame.mixer.music.pause();
                                playing = False;
                         frame = Bucky(music_files[playing_index], parent = None, id = -1);
                         frame.Show();
                            
                     else :
                        buttons["fm"].setImage('./image/fm_start.png');
                        fmworking = False;
                        os.system('echo 123');
        ...
        #写歌名
        if max_track == 0:
            label = font.render(("文件夹中没有音乐 ").decode('utf-8'), True, (255, 255, 255));
        ...
        if vol_change:
            ...
            label = vfont.render(("音量: "+str(now_vol)).decode('utf-8'), True, (255, 255, 255))
            w, h = label.get_size();
        ...
        if suprise:
            ...
            if s_setp > 40:
                suprise = False;
            label = vfont.render("按下Up/Down有惊喜噢".decode("utf-8"), True, (255, 255, 255))
            w, h = label.get_size();
